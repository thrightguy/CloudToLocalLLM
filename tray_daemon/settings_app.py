...
class TrayDaemonSettings:
    ...
    def _create_widgets(self):
        ...
        ttk.Button(
            buttons_frame, text="Save", command=self._save_settings
        ).pack(side=tk.RIGHT, padx=(5, 0))
        ttk.Button(
            buttons_frame, text="Test Connections", command=self._test_connections
        ).pack(side=tk.RIGHT)
        ttk.Button(
            buttons_frame, text="Refresh Status", command=self._refresh_status
    ...
    def _create_ollama_tab(self):
        ...
        ttk.Checkbutton(
            self.ollama_frame, text="Enable Local Ollama Connection",
        ...
        ttk.Entry(
            host_frame, textvariable=self.ollama_host, width=20
        ).pack(side=tk.LEFT, padx=(5, 10))

        ttk.Label(host_frame, text="Port:").pack(side=tk.LEFT)
        self.ollama_port = tk.StringVar()
        ttk.Entry(
            host_frame, textvariable=self.ollama_port, width=10
        ...
        ttk.Entry(
            timeout_frame, textvariable=self.ollama_timeout, width=10
        ).pack(side=tk.LEFT, padx=(5, 0))
    ...
    def _create_cloud_tab(self):
        ...
        ttk.Checkbutton(
            self.cloud_frame, text="Enable Cloud Proxy Connection",
            variable=self.cloud_enabled
        ).pack(anchor=tk.W, pady=5)
        ...
        ttk.Entry(
            url_frame, textvariable=self.cloud_url, width=50
        ...
        token_entry = ttk.Entry(
            token_frame, textvariable=self.cloud_token, width=50, show="*"
        )
        ...
        show_btn = ttk.Checkbutton(
            token_frame, text="Show token", variable=self.show_token,
        ...
        ttk.Entry(
            timeout_frame, textvariable=self.cloud_timeout, width=10
        ).pack(side=tk.LEFT, padx=(5, 0))
    ...
    def _create_status_tab(self):
        ...
        self.status_text = scrolledtext.ScrolledText(
            self.status_frame, height=20, width=70
        )
        ...
        ttk.Checkbutton(
            self.status_frame, text="Auto-refresh every 5 seconds",
    ...
    def _load_settings(self):
        ...
        self.cloud_url.set(
            cloud_config.get("api_base_url", "https://api.cloudtolocalllm.online")
        )
    ...
    def _save_settings(self):
        ...
        try:
            ...
            self.config["local_ollama"].update({
                ...
                "api_base_url": (
                    f"http://{self.ollama_host.get()}:{self.ollama_port.get()}"
                )
            })
    ...
    def _test_ollama_connection(self):
        ...
        try:
            ...
            response = requests.get(
                url, timeout=int(self.ollama_timeout.get())
            )
            ...
            if response.status_code == 200:
                ...
                self.status_text.insert(
                    tk.END, f"✓ Local Ollama: Connected (v{version})\n"
                )
                self.ollama_status.config(
                    text=f"Status: Connected (v{version})"
                )
            else:
                self.status_text.insert(
                    tk.END, f"✗ Local Ollama: HTTP {response.status_code}\n"
                )
                self.ollama_status.config(
                    text=f"Status: Error (HTTP {response.status_code})"
                )
    ...
    def _test_cloud_connection(self):
        ...
        try:
            ...
            response = requests.get(
                url, headers=headers, timeout=int(self.cloud_timeout.get())
            )
            ...
            if response.status_code == 200:
                ...
                self.status_text.insert(
                    tk.END, f"✗ Cloud Proxy: HTTP {response.status_code}\n"
                )
