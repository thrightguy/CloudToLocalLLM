...
class ConnectionBroker:
    ...
    async def _check_connection(self, connection_type: ConnectionType):
        ...
        try:
            ...
            self.logger.error(f"Connection check failed for {connection_type.value}: {e}")
        
        status.last_check = time.time()
        
        # Notify if state changed
        if old_state != status.state:
            self.logger.info(f"Connection {connection_type.value} state: {old_state.value} -> {status.state.value}")
    ...
    def get_best_connection(self) -> Optional[ConnectionType]:
        ...
        if (self.connections[ConnectionType.LOCAL_OLLAMA].enabled and
            self.connection_status[ConnectionType.LOCAL_OLLAMA].state == ConnectionState.CONNECTED):
            return ConnectionType.LOCAL_OLLAMA

        # Fall back to cloud proxy if available
        if (self.connections[ConnectionType.CLOUD_PROXY].enabled and
            self.connection_status[ConnectionType.CLOUD_PROXY].state == ConnectionState.CONNECTED):
    ...
    async def proxy_request(self, method: str, path: str, data: Dict[str, Any] = None,
    ...
    async def _proxy_ollama_request(self, config: ConnectionConfig, method: str,
    ...
    async def _proxy_cloud_request(self, config: ConnectionConfig, method: str,
                                  path: str, data: Dict[str, Any] = None) -> Dict[str, Any]:
    ...
    async def _stream_ollama_chat(self, config: ConnectionConfig, model: str,
    ...
    async def _stream_cloud_chat(self, config: ConnectionConfig, model: str,
                                messages: List[Dict[str, str]]):
