...
# First gets a regular certificate using HTTP validation, then a wildcard using DNS validation.
...
run_webroot_certbot() {
    echo_color "$BLUE" "Step 1: Obtaining regular certificate for $DOMAIN_NAME using HTTP validation..."
    ...
    if [ $certbot_exit_code -eq 0 ]; then
        echo_color "$GREEN" "Regular certificate obtained successfully!"
    else
...
run_dns_certbot() {
    echo_color "$BLUE" "Step 2: Obtaining wildcard certificate for *.${DOMAIN_NAME} using DNS validation..."
