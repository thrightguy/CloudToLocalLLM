...
# with DNS validation for wildcard certificates.
...
run_certbot_command() {
    echo_color "$BLUE" "Attempting to obtain/renew wildcard certificate for $DOMAIN_NAME using DNS validation..."
    ...
    if [ $certbot_exit_code -eq 0 ]; then
        echo_color "$GREEN" "Staging certificate obtained successfully!"
        echo_color "$YELLOW" "To obtain a production certificate, run this script again with --production flag."
    else
        echo_color "$RED" "Failed to obtain staging certificate. Check the logs in $CERTBOT_LOG_DIR"
    fi
