...
class VersionService {
  ...
  Future<void> _loadVersionInfo() async {
    try {
      // Try to load from version.json asset first (generated during build)
      await _loadFromAsset();
    } catch (e) {
      debugPrint('[VersionService] Could not load from asset: $e');
  ...
  Future<void> _loadFromAsset() async {
    try {
      final String versionJson = await rootBundle.loadString('assets/version.json');
  ...
  Future<void> _loadFromPubspec() async {
    try {
      ...
      const String compiledVersion = '2.0.0'; // Will be replaced by build script
      const String compiledBuildNumber = '202506011'; // Will be replaced by build script
  ...
  DateTime? _parseBuildDate(String buildNumber) {
    try {
      ...
      if (buildNumber.length == 12 && RegExp(r'^\d{12}$').hasMatch(buildNumber)) {
  ...
  Future<String> getAboutVersion() async {
    ...
    if (buildDate != null) {
      final formattedDate = '${buildDate.day}/${buildDate.month}/${buildDate.year}';
