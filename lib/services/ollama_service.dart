...
class OllamaService extends ChangeNotifier {
  ...
  Future<bool> testConnection() async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        _isConnected = true;
        debugPrint('Connected to Ollama v$_version');
        return true;
      } else {
  ...
  Future<List<OllamaModel>> getModels() async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        _models = modelsList.map((model) => OllamaModel.fromJson(model)).toList();
        debugPrint('Found ${_models.length} Ollama models');
        return _models;
      } else {
  ...
  Future<bool> pullModel(String modelName) async {
    try {
      ...
      ).timeout(const Duration(minutes: 10)); // Longer timeout for model downloads
