...
Write-ColorOutput Green "Setting up Docker web-only environment on $VpsHost..."
...
# Create Docker-only setup script with direct port mapping - WITHOUT OLLAMA
...
services:
  ...
  # Cloud service only, no Ollama or Tunnel
  cloud:
    image: node:20
    working_dir: /app
    volumes:
...
echo -e "${YELLOW}Setting up Docker web-only environment (no Ollama)...${NC}"
...
# Create a setup script for cloud
cat > setup_cloud.sh << 'EOF'
#!/bin/bash
set -e

# Start node server for cloud component
cd /app
...
echo -e "${GREEN}Docker web-only setup with SSL completed successfully!${NC}"
...
$tempDir = Join-Path $env:TEMP "cloudtolocalllm_web_only"
if (Test-Path $tempDir) { Remove-Item -Recurse -Force $tempDir }
New-Item -ItemType Directory -Path $tempDir | Out-Null

$dockerComposePath = Join-Path $tempDir "docker-compose.yml"
$setupScriptPath = Join-Path $tempDir "docker_web_only_setup.sh"
...
Invoke-ScpCommand $setupScriptPath "~/docker_web_only_setup.sh"

# Fix line endings, make executable, and run
Write-ColorOutput Yellow "Running Docker web-only setup script..."
Invoke-SshCommand "sed -i 's/\r$//' ~/docker_web_only_setup.sh && chmod +x ~/docker_web_only_setup.sh && ./docker_web_only_setup.sh"

# Clean up
Remove-Item -Recurse -Force $tempDir

Write-ColorOutput Green "Docker web-only setup completed successfully!"
Write-ColorOutput Yellow "Your website is now accessible via HTTPS without Ollama:"
